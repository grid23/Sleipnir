...
(function(root){ "use strict"
    var document = root.document
      ...
      , browserMask = env.browser = klass(Model, function(_, supr){
            ...
            return {
                _construct: function(){
                    ...
                    this.set('oldIE', oldIE)
                    
                    this.set('width', root.innerWidth)
                    this.set('height', root.innerHeight)
                    
                    _.dom.addEventListener(root, 'resize', function(e){
                        resizeTimer = setTimeout(function(){
                            self.set('width', root.innerWidth)
                            self.set('height', root.innerHeight)
                            self.emit('browser.resize', e)
                        }, 4)
                    }, true)
      ...
      , deviceMask = env.device = klass(Model, function(_, supr){
            ...
            var screen = root.screen
              ...
              , touch = root.hasOwnProperty('ontouchstart') && root.hasOwnProperty('ontouchend')
            ...
            return {
                _construct: function(){
                    ...
                    this.set('pixelRatio', pixelRatio)
                    this.set('retina', !!(pixelRatio == 2))
                    this.set('hdpi', !!(pixelRatio > 1))
                    this.set('touch', touch)
                    
                    this.set('orientation', orientation)
                    _.dom.addEventListener(root, "orientationchange", function(e){
                        self.emit('device.orientationchange', e)
                        self.set('orientation', root.orientation)
                    })
                }
            }
        }, true)
      
      , viewport = env.viewport = klass(Model, function(_, supr){
            ...
            return {
              ...
              , serialize: function(){
                    var str = []
                    for ( var i=0, keys=_.keys(this.variables), l=keys.length; i<l; i++ )
                      str.push( keys[i]+'='+this.get(keys[i]))
                    return str.join(', ')
                }
              , inserted: 0
              , insert: function(){
                    ...
                    var head = document.head
                      , pos = document.head.childNodes[0]
                    head.insertBefore(this.node, pos)
                    
                    this.inserted++
                    return this
                }
              , update: function(){
                    this.node.content = this.serialize()
                    return this
                }
            }
        }, true)
